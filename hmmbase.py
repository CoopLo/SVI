...
import pysvihmm.hmm_fast as hmm_fast
import pysvihmm.util as util
...
class VariationalHMMBase(object):
    ...
    def __init__(self, obs, prior_init, prior_tran, prior_emit, mask=None,
        ...
        # it looks like the forward-backward message stuff was forgotten
        self.lalpha = np.ndarray((5, obs.size))
        self.lbeta = np.ndarray((5, obs.size))
    ...
    def local_update(self, obs=None, mask=None):
        ...
        # Compute likelihoods
        self.lliks = np.ndarray((5, self.obs.size))
    ...
    def backward_msgs(self, obs=None, mask=None):
        ...
        for t in range(self.T-2,-1,-1):
            lbeta[:,t] = np.logaddexp.reduce(ltran + lbeta[:,t+1] + ll[:,t+1], axis=1)
