...
for i in range(1, train_size):
    if(train_data[i] < (1.005*train_data[i-1]) and train_data[i] > (0.995*train_data[i-1])):
        prior_tran[0][state] += 1
        state = 0
    elif(train_data[i] >= (1.005*train_data[i-1]) and train_data[i] < (1.02*train_data[i-1])):
        prior_tran[1][state] += 1
        state = 1
    elif(train_data[i] >= (1.02*train_data[i-1])):
        prior_tran[2][state] += 1
        state = 2
    elif(train_data[i] <= (0.995*train_data[i-1]) and train_data[i] > (0.98*train_data[i-1])):
        prior_tran[3][state] += 1
        state = 3
    elif(train_data[i] <= (0.98*train_data[i-1])):
